...
int main(int argc, char **argv)
{
	...
	while (running)
	{
		...
		if (raytracing)
		{
			glm::vec3 xBasis = glm::normalize(glm::cross(up, dir));
			glm::vec3 yBasis = glm::normalize(glm::cross(dir, xBasis));


			float height = round(SCR_HEIGHT * tan(fovy));
			float width = round(height);
			...
			for (int y = 0; y < height; y++)
			{
				...
				for (int x = 0; x < width; x++)
				{
					...
					float yFac = -2.0 * (float(y) / height) + 1.0;
					float xFac = -2.0 * (float(x) / width) + 1.0;
					glm::vec3 currentDir = glm::normalize(dir + (xFac * xBasis) + (yFac * yBasis));
